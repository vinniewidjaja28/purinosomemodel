...
AMCPMean=mean(A);
rPQ=2*p.VPQ*A*1000/(p.KPQ+A*1000)*(p.VMCP/1000);
...
if plotSwitch
    ...
    xlim([0 .00105]) %change the xlim to actual radius (10.5 uM)
    ylim([1e-16 100]) % change to double the highest value (half lower value for lower lim) 
