...
rPQ=2*p.VPQ*Afull*1000/(p.KPQ+Afull*1000)*(p.VMCP/1000);
baselineFlux=rPQ/(10^6)*(58.1*3600)/2;
...
for ii = 1:length(params)
    ...
    flux1(ii)=rPQ(ii)/(10^6)*(58.1*3600)/2;
    tDouble1(ii)=0.3*10^-12/flux1(ii);
...
for ii = 1:length(params)
    ...
    Acyto(ii)=res.a_cyto_uM/1000;
    rPQ(ii)=2*p.VPQ*Afull(ii)*1000/(p.KPQ+Afull(ii)*1000)*(p.VMCP/1000);
    flux2(ii)=rPQ(ii)/(10^6)*(58.1*3600)/2;
    tDouble2(ii)=0.3*10^-12/flux2(ii);
